...
public class ResultadosPesquisaActivity  extends AppCompatActivity {
    ...
    public interface RequestService {
        @GET("/v1/search")
        Call<List<ResultadosPesquisaActivity.Pesquisa>> search(
    ...
    public void fetchRequisicoes(String search){
        ...
        ResultadosPesquisaActivity.RequestService service = retrofit.create(ResultadosPesquisaActivity.RequestService.class);

        Log.d("Pesquisa", "Search query: " + search);

        Call<List<ResultadosPesquisaActivity.Pesquisa>> call = service.search(search);
        Log.d("Pesquisa", "URL: " + call.request().url().toString());
        call.enqueue(new Callback<List<ResultadosPesquisaActivity.Pesquisa>>() {
            @Override
            public void onResponse(Call<List<ResultadosPesquisaActivity.Pesquisa>> call, Response<List<ResultadosPesquisaActivity.Pesquisa>> response) {
                if (response.isSuccessful()) {
                    List<ResultadosPesquisaActivity.Pesquisa> searches = response.body();
            ...
            @Override
            public void onFailure(Call<List<ResultadosPesquisaActivity.Pesquisa>> call, Throwable t) {
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        if (position != RecyclerView.NO_POSITION) {
            ResultadosPesquisaActivity.Pesquisa pesquisa = adapter.pesquisas.get(position);
